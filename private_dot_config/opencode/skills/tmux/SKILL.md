...
```bash
SOCKET_DIR=${TMPDIR:-/tmp}/opencode-tmux-sockets  # well-known dir for all agent sockets
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/opencode.sock"                # keep agent sessions separate from your personal tmux
SESSION=opencode-python                           # slug-like names; avoid spaces
...
To monitor this session yourself:
  tmux -S "$SOCKET" attach -t opencode-lldb

Or to capture the output once:
  tmux -S "$SOCKET" capture-pane -p -J -t opencode-lldb:0.0 -S -200
```
...
- Agents MUST place tmux sockets under `opencode_TMUX_SOCKET_DIR` (defaults to `${TMPDIR:-/tmp}/opencode-tmux-sockets`) and use `tmux -S "$SOCKET"` so we can enumerate/clean them. Create the dir first: `mkdir -p "$opencode_TMUX_SOCKET_DIR"`.
- Default socket path to use unless you must isolate further: `SOCKET="$opencode_TMUX_SOCKET_DIR/opencode.sock"`.

## Targeting panes and naming

- Target format: `{session}:{window}.{pane}`, defaults to `:0.0` if omitted. Keep names short (e.g., `opencode-py`, `opencode-gdb`).
...
- Scan all sockets under the shared directory: `./scripts/find-sessions.sh --all` (uses `opencode_TMUX_SOCKET_DIR` or `${TMPDIR:-/tmp}/opencode-tmux-sockets`).

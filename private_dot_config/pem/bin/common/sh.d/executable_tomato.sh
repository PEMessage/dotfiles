...
case $(uname -a) in *WSL2*) WSL2="true";; esac
if [ "$WSL2" = true ] ; then
    notify-send() { wsl-notify-send.exe --category tomato "${@}"; }
fi
...
countdown(){
  ...
  while : ; do
    ...
    notify-send -t ${expiryTimeMs} "${message}: ${sinceMin}m${sinceSec}s"
    sleep ${tickPeriod}
  done
  notify-send -t 1 "${message}: Countdown done!!"
}
